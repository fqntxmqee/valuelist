...
import net.mlw.vlh.adapter.util.FilterTextManipulator;
import net.mlw.vlh.adapter.util.TextManipulator;
import net.mlw.vlh.adapter.util.TokenReplaceTextManipulator;
...
public class ConfigurableStatementBuilder implements StatementBuilder
{
   ...
   public void init()
   {
      ...
      {
         textManipulators = new LinkedList();
    ...
    */
   public PreparedStatement generate(Connection conn, StringBuffer query, Map whereClause, boolean scrollable) throws SQLException,
         ParseException
   {
      if (!init)
      {
      ...
      if (scrollable)
      {
         statement = conn.prepareStatement(query.toString(), ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
      }
      else
      {
